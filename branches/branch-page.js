...
class BranchMonitorApp {
  ...
  async init() {
    ...
    try {
      ...
      // Also check dynamic projects from LocalStorage
      let allProjects = pRes;
      const localProjects = localStorage.getItem('wma_ecosystem_projects_v3');
      if (localProjects) {
        try {
  ...
  renderBranchInfo() {
    ...
    if (tableEl) {
      tableEl.innerHTML = `
        ...
        <tr><th>지부장</th><td>${b.leader}</td></tr>
        <tr><th>사무국장</th><td>${b.manager}</td></tr>
      `;
    }
  ...
  renderProjectsList() {
      ...
      container.innerHTML = `
        <div class="empty-state-box">
          ...
          <div class="empty-state-desc">
            현재 본 지부에 할당된 신규 사업이 없습니다.<br>
            중앙사무국 총괄 시스템에서 사업이 개설되면 여기에 실시간 표출됩니다.
          </div>
        </div>
      `;
      return;
    ...
    container.innerHTML = this.projectsData.map(p => `
      <div style="background: rgba(6,9,14,0.8); border: 1px solid var(--border-subtle); border-radius: 4px; padding: 10px; margin-bottom: 8px;">
        <div style="font-size: 0.82rem; font-weight: 800; color: #34d399; margin-bottom: 4px;">🌿 ${p.title}</div>
        ...
        <div style="font-size: 0.74rem; font-weight: 700; color: #38bdf8;">실적: ${(Number(p.total_area_m2)).toLocaleString()}㎡ / ${p.total_harvest_kg}kg</div>
        ${p.live_dashboard_url ? `<a href="${p.live_dashboard_url}" target="_blank" class="btn-tactical primary" style="width: 100%; justify-content: center; margin-top: 6px; font-size: 0.72rem;">🚀 3D 드론 정사영상 열기</a>` : ''}
      </div>
    `).join('');
  }
    ...
    this.map.on('load', () => {
      ...
      branchWrap.innerHTML = `
        <svg width="96" height="62" viewBox="0 0 96 62" fill="none" xmlns="http://www.w3.org/2000/svg" style="display:block; overflow:visible; filter: drop-shadow(0 3px 6px rgba(0,0,0,0.9));">
          <rect x="6" y="2" width="84" height="22" rx="4" fill="#082f49" stroke="#38bdf8" stroke-width="1.5"/>
          <text x="48" y="17" text-anchor="middle" fill="#ffffff" font-family="-apple-system, sans-serif" font-size="11.5" font-weight="800">
            🏛️ ${b.short_name}
          </text>
      ...
      this.projectsData.forEach(p => {
        ...
        pWrap.innerHTML = `
          <svg width="96" height="62" viewBox="0 0 96 62" fill="none" xmlns="http://www.w3.org/2000/svg" style="display:block; overflow:visible; filter: drop-shadow(0 3px 6px rgba(0,0,0,0.9));">
            <rect x="6" y="2" width="84" height="22" rx="4" fill="#022c22" stroke="#34d399" stroke-width="1.5"/>
            <text x="48" y="17" text-anchor="middle" fill="#a7f3d0" font-family="-apple-system, sans-serif" font-size="11.5" font-weight="800">
              🌿 ${p.title.slice(0, 5)}...
            </text>
